...
func BenchmarkMergeWorst(b *testing.B) {
	for i := 0; i < b.N; i++ {
		mergeSort(cases[0].input)
	}
}

func BenchmarkMergeAverage(b *testing.B) {
	for i := 0; i < b.N; i++ {
		mergeSort(cases[1].input)
	}
}

func BenchmarkMergeBest(b *testing.B) {
	for i := 0; i < b.N; i++ {
		mergeSort(cases[2].input)
	}
}
